...
export default function SummonerPage({ summonerData, rankData, matchesData, matchDataArray }) {
    ...
    const [regionCounter, setRegionCounter] = React.useState("LAS")
    const [openedModal,
...
export async function getServerSideProps({ params, res }) {
    ...
    const summonerResponse = await fetch(`https://la2.api.riotgames.com/lol/summoner/v4/summoners/by-name/${fixedSummonerName}?api_key=${process.env.RIOT_API}`)
    ...
    const rankResponse = await fetch(`https://la2.api.riotgames.com/lol/league/v4/entries/by-summoner/${summonerData.id}?api_key=${process.env.RIOT_API}`)
