...
export default function SummonerPage({ summonerData, rankData, matchesData, matchDataArray }) {
    ...
    const [regionCounter, setRegionCounter] = React.useState("EUNE")
    const [openedModal,
...
export async function getServerSideProps({ params, res }) {
    ...
    const summonerResponse = await fetch(`https://eun1.api.riotgames.com/lol/summoner/v4/summoners/by-name/${fixedSummonerName}?api_key=${process.env.RIOT_API}`)
    const summonerData = await summonerResponse.json()
    const matchesResponse = await fetch(`https://europe.api.riotgames.com/lol/match/v5/matches/by-puuid/${summonerData.puuid}/ids?start=0&count=5&api_key=${process.env.RIOT_API}`)
    const matchesData = await matchesResponse.json()
    const rankResponse = await fetch(`https://eun1.api.riotgames.com/lol/league/v4/entries/by-summoner/${summonerData.id}?api_key=${process.env.RIOT_API}`)
